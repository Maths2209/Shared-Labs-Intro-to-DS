...
The plot illustrates how each of the predictor's coefficients change in response to a decrease in lasso regularization parameter. We can tell that the lasso regularization parameter decreases as the x-axis increases from 0 to 1. This is because we know that as regularization parameter increases, more and more coefficients are regularized into 0.
...
It is better to use nstart = 25 than the function's default of nstart = 1 because the process of initializing the starting centroids is random. More often than not, different initialization render rather different equilibrium. Hence comparing 25 different runs and obtaining the best one will very likely give us more accurate clustering than simply one attempt.
...
We choose 3 to be the value k, as that point seems to be the "elbow" of the above WSS plot. The reduction in RSS from 1 to 2 and from 2 to 3 are fairly significant. The reduction in WSS is rather linear and not as drastic for k>3. 
...
Cluster 1: Relatively lower serum concentration of lamorigine (LTG) and relatively lower body mass index (BMI) 

Cluster 2: Relatively higher serum concentration of lamorigine (LTG) but relatively lower body mass index (BMI) 

Cluster 3: Relatively higher serum concentration of lamorigine (LTG) and relatively higher body mass index (BMI) 
