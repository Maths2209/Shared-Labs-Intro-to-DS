...
```{r}
contentCorpus <- wc$Review.Text

# Create Corpus
ccorpus <- VCorpus(VectorSource(contentCorpus))
...
```{r}
min_terms <- findFreqTerms(dtm, lowfreq = 10, highfreq = 20)
min_m <- as.matrix(dtm)
...
logit <- glm(wc$tf_idf ~ ., family=binomial)
summary(logit)
