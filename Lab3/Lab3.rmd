...
The negative sign indicates that a person with a greater monthly income will have a lower probability of experiencing the financial distress of experiencing 90 days past due delinquency or worse. The negative sign is expected and makes intuitive sense.
...
The two box plots are very similar which indicates a massive overlap in the distribution of probabilities for those who had a serious delinquency and those who did not. The median for the group of people who experienced 90 days past due delinquency or worse have a slightly higher median and interquartile range, but this difference is not clear just from these boxplots. Therefore, the model is not very useful as it does not distinguish well between the two groups which makes it likely that it will misclassify many people.
...
The ROC curve is now much further above the line y=x (in red) and the AUC is now 0.6912 which is greater than the original 0.576 and closer to 1. These indicate that the model is making better predictions of the outcome than before. This is to be expected since the model is drawing upon more data to make its predictions (since we added more predictors). The increase in the AUC indicates that the additional predictors indeed play a role in determining the outcome. 
...
The coefficients for the logistic regression fit are determined by the maximum likelihood estimation (MLE) method. The MLE method finds the coefficient for each predictor, under which the observed y outcome are most likely. We use the log-likelihood function to determine the coefficient values that maximize it. 
