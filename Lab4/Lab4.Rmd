...
The slope is 0.023619 with regards workingday. That means, with one unit increase in the workingday, count is multiplied by a factor of e^0.023619, which is 1.0239.
...
The slope is not practically significant at all. As mentioned above, for every one unit increase in workingday, count increases by a factor of 1.0239. Workingday ranges from 0 to 1, and so for this range, the range of count as impacted by workingday is only 0 to 1.0239, which is almost negilibile compared the full range of count from 1-977. 
...
The slope is 0.0236. The interpretation for this slope is that, with a unit increase in workingday, count increases by a factor of 1.023881 times. The interpretation does not change much, since both poisson and negative binomial model have the same link function from the perspective of GLM. 
